...
<head>
    ...
    <title>{{ config('app.name', 'TranquilMinds') }}</title>
...
<body>
        ...
        <nav class="navbar navbar-expand-md navbar-light bg-white shadow-sm">
            ...
            <div class="collapse navbar-collapse" id="navbarSupportedContent">
                ...
                <ul class="navbar-nav ml-auto">
                    ...
                    <li class="nav-item">
                        <a style="color: #fff; font-size:16px; font-weight: bold;" class="nav-link" href="{{ route('take.test') }}">{{ __('Mental Health Test') }}</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link" href="{{ route('my.booking') }}" style="color: #fff; font-size:16px; font-weight: bold;">{{ __('My Booking History') }}</a>
                    </li>
                    @endif
                    @if(auth()->check() && auth()->user()->role->name === 'patient')
                    <li class="nav-item">
                        <a style="color: #fff; font-size:16px; font-weight: bold;" class="nav-link" href="{{ route('my.prescription') }}" style="color: #fff; font-size:16px; font-weight: bold;">{{ __('My Appointment Report') }}</a>
                    </li>
                    @endif
    ...
    <style type="text/css">
        ...
        .navbar {
            background: rgb(14, 9, 96);
            background: linear-gradient(90deg, rgba(14, 9, 96, 1) 0%, rgba(30, 209, 213, 0.6287128712871287) 0%, rgba(19, 127, 142, 1) 0%, rgba(22, 148, 160, 1) 75%);
        }
    </style>
